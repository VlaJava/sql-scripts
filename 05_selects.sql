...
-- Venda por período destino e cliente.
...
FROM 
	TB_USERS U 
LEFT JOIN TB_ROLES RL ON U.ID = RL.USER_ID;
...
SELECT P.TITLE, 
       AVG(CAST(A.RATING AS DECIMAL(10,2))) AS AVERAGE_RATING, 
       COUNT(A.ID) AS QUANTIDADE_REVIEWS
FROM TB_PACKAGES P
LEFT JOIN TB_BOOKINGS R ON P.ID = R.PACKAGE_ID
LEFT JOIN TB_REVIEWS A ON R.ID = A.BOOKING_ID  
GROUP BY P.TITLE
...
SELECT P.TITLE, 
       AVG(CAST(A.RATING AS DECIMAL(10,2))) AS MEDIA_AVALIACAO
FROM TB_PACKAGES P
JOIN TB_BOOKINGS R ON P.ID = R.PACKAGE_ID -- Corrigido
JOIN TB_REVIEWS A ON R.ID = A.BOOKING_ID  -- Corrigido
GROUP BY P.TITLE
HAVING AVG(CAST(A.RATING AS DECIMAL(10,2))) < (SELECT AVG(CAST(RATING AS DECIMAL(10,2))) FROM TB_REVIEWS)
...
SELECT P.TITLE, P.PRICE, 
       AVG(CAST(A.RATING AS DECIMAL(10,2))) AS MEDIA_AVALIACAO,
       (AVG(CAST(A.RATING AS DECIMAL(10,2))) / P.PRICE) AS CUSTO_BENEFICIO
FROM TB_PACKAGES P
JOIN TB_BOOKINGS RS ON P.ID = RS.PACKAGE_ID -- Corrigido
...
SELECT 
  ...
  (SELECT COUNT(*) FROM TB_BOOKINGS WHERE BOOKING_STATUS = 'CONFIRMED') AS CONFIRMED_BOOKINGS,
  (SELECT SUM(TOTAL_PRICE) FROM TB_BOOKINGS WHERE BOOKING_STATUS = 'CONFIRMED') AS TOTAL_REVENUE,
...
-- Pacotes que a data de início foi no passado  
