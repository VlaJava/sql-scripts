...
(
  ...
  CHECK(DOCUMENT_TYPE = 'CPF' OR DOCUMENT_TYPE = 'PASSAPORTE')
)
END	
GO
...
(
  ...
  CHECK(UPPER(BOOKING_STATUS) IN ('PENDING', 'CONFIRMED', 'REJECTED', 'CANCELED'))
)
END
GO
...
(
  ...
  CHECK(UPPER(METHOD) IN ('PIX', 'DEBIT', 'CREDIT', 'BOLETO')),
  CHECK(AMOUNT > 0),
  CHECK(UPPER(PAYMENT_STATUS) IN ('PENDING','APPROVED', 'REJECTED', 'REFUNDED'))
)
END
GO
