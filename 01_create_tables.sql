...
(
  ...
  CHECK(LEN(NOME) >= 3),
  CHECK(LEN(SENHA) > 5),
  CHECK (DATEDIFF(YEAR, DATA_NASC, GETDATE()) - 
       CASE 
         WHEN (MONTH(GETDATE()) < MONTH(DATA_NASC)) 
              OR (MONTH(GETDATE()) = MONTH(DATA_NASC) AND DAY(GETDATE()) < DAY(DATA_NASC)) 
...
(
  ...
  CHECK(UPPER(METODO) IN ('PIX', 'DEBITO','CREDITO', 'BOLETO')),
  CHECK(VALOR_PAGO > 0),
